        ...
        <div class="category-tab-content">
            <ul class="normal-nav">
                <c:forEach items="${categoryList}" var="category">
                    <li class="nav-item" category_id="${category.id}">${category.name}</li>
                </c:forEach>
            </ul>
...
<div class="new-floor-con">
    <div class="workArea">
        <c:forEach items="${categoryList}" var="category" varStatus="status">
            ...
            <%-- 该分类下有产品才能显示 --%>
            <!-- 种类对应多个商品，我们的数据库默认只有类别1对应了多个商品 -->
            <c:if test="${!empty category.productList}">
                <%-- 默认只展示前五个分类的内容，多了页面太长 --%>
                <c:if test="${status.count<=5}">
                    <div class="floor-line-con">
                        <i class="color-mark"></i>
                        <div class="floor-name">${category.name}</div>
                        <br>
                        <!-- 遍历种类中所有的商品 -->
                        <c:forEach items="${category.productList}" var="product" varStatus="status">
                            <c:if test="${status.count<=5}">
                                <a class="grid" href="productDetail.action?product_id=${product.id}">
                                    <div class="productItem">
                                        <img class="floor-item-img" src="img/product/${product.id}/1.jpg">
                                        <div class="floor-item-title">${product.name}</div>
                                        <div class="floor-price">${product.price}</div>
                                    </div>
                                </a>
                            </c:if>
                        </c:forEach>
